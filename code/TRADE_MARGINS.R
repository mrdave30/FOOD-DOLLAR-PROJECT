...
output_dir <- file.path(main_dir, "Processed_files")
dir_create(output_dir)
final_output_dir <- file.path(main_dir, "Final_Trade_Margin")
...
process_csv_files <- function(file_path) {
  ...
  output_final <- file.path(final_output_dir, paste0(Year, "_", "Trade_Margin.csv"))
...
#-----------------convert dta to csv-------------
read.csv("2007_IOT.csv") -> data

write_dta(data, "2007_IOT.dta")

read.csv("2007_FTAH.csv") -> FTAH
write_dta(FTAH, "FTAH.dta")
read.csv("2007_FAH.csv") -> FAH
write_dta(FAH, "FAH.dta")

getwd()
# #merge inputoutput with country name
# 
...
# 
# write_dta(input, "1990_Input_Output.dta")
